...
namespace HIT_IR_LTP{
	...
	int XML4NLP::CreateDOMFromFile( const char* fileName )
	{
		...
		{
			clean_str(line); // Zhenghua Li, 2007-8-31, 15:57	去掉前后空格
			remove_space(line);		//
			if (line.empty()) continue; // Blank line. Is this the best way?
	...
	int XML4NLP::CreateDOMFromString(const string& str)
	{
		...
		string strTmp = str;
		replace_char_by_char(strTmp, '\r', 'n');
		...
		while (getline(in, strTmp))
		{
			...
			if (0 != BuildParagraph(strTmp, i++)) return -1;
		}
	...
	*/
	...
	const char* XML4NLP::GetOldWord(int paragraphIdx, int sentenceIdx, int wordIdx) const
	{
	...
	const char* XML4NLP::GetOldWord(int globalSentIdx, int wordIdx) const
	{
		pair<int, int> paraIdx_sentIdx;
	...
	const char* XML4NLP::GetOldWord(int globalWordIdx) const
	{
	...
	//	int i = 0;
		do
		{
			...
			vecBegEnd.push_back(pair<int, int>(uiBeg, uiEnd));//[i].first = uiBeg;
	...
	/*
		...
		do
		{
			...
			vecBegEnd.push_back(pair<int, int>(uiBeg, uiEnd));
	...
	//	TiXmlDeclaration* xmlDeclaration = new TiXmlDeclaration("1.0", "gb2312", "");
		TiXmlDeclaration* xmlDeclaration = new TiXmlDeclaration("1.0", encoding.c_str(), "");
	...
	int XML4NLP::BuildParagraph(string& strParagraph, int paragraphIdx)
	{
		...
		vecParagraph_t.push_back( Paragraph_t() );
		Paragraph_t &paragraph = vecParagraph_t[vecParagraph_t.size() - 1];//last element, which means Paragraph_t()
	...
	int XML4NLP::GetInfoFromSentence(vector<const char *> &vecInfo, int paragraphIdx, int sentenceIdx, const char *attrName) const
	{
		if (0 != CheckRange(paragraphIdx, sentenceIdx)) return -1;

		const vector<Word_t> &vecWord_t = m_document_t.vecParagraph_t[paragraphIdx].vecSentence_t[sentenceIdx].vecWord_t;

		vecInfo.resize(vecWord_t.size());
	/*
	...
	//************************************
	// Method:    GetWord
	// FullName:  XML4NLP::GetWord
	// Access:    public 
	// Returns:   Word
	// Qualifier:
	// Parameter: int paragraphIdx
	// Parameter: int sentenceIdx
	// Parameter: int wordIdx
	//************************************
	Word XML4NLP::GetWord( int paragraphIdx, int sentenceIdx, int wordIdx)
	{
		...
		Word w(*(m_document_t.vecParagraph_t[paragraphIdx].vecSentence_t[sentenceIdx].vecWord_t[wordIdx].wordPtr));
		return w;
	}
	...
	// Returns:   Word
	// Qualifier:
	// Parameter: int globalWordIdx
	//************************************
	Word XML4NLP::GetWord( int globalWordIdx)
	{
		int paraIdx, sentIdx, wordIdx;
		if (0 != MapGlobalWordIdx2paraIdx_sentIdx_wordIdx(globalWordIdx, paraIdx, sentIdx, wordIdx)) 
		{
			Word w;
			cerr<<"GetWord(int) out of range"<<endl;
			return w;
		}
		Word w(*(m_document_t.vecParagraph_t[paraIdx].vecSentence_t[sentIdx].vecWord_t[wordIdx].wordPtr));
		return w;
	}

	//************************************
	// Method:    GetWords
	// FullName:  XML4NLP::GetWords
	// Access:    public 
	// Returns:   bool
	// Qualifier:
	// Parameter: vector<Word> & wordList
	// Parameter: int paragraphIdx
	// Parameter: int sentenceIdx
	//************************************
	bool XML4NLP::GetWords( vector<Word> & wordList, int paragraphIdx, int sentenceIdx)
	{
		if (0 != CheckRange(paragraphIdx, sentenceIdx) ) return false;
		vector<Word_t> &eleList = m_document_t.vecParagraph_t[paragraphIdx].vecSentence_t[sentenceIdx].vecWord_t;
	...
	bool XML4NLP::GetWordsFromSent( vector<Word> &wordList, int globalSentIdx)
	{
		pair<int, int> paraIdx_sentIdx;
		if (0 != MapGlobalSentIdx2paraIdx_sentIdx(globalSentIdx, paraIdx_sentIdx)) return false;

		vector<Word_t> &eleList = m_document_t.vecParagraph_t[paraIdx_sentIdx.first].vecSentence_t[paraIdx_sentIdx.second].vecWord_t;
		for (int i = 0; i< eleList.size(); ++i)
		{
			Word w(*(eleList[i].wordPtr));
			wordList.push_back(w);
	...
	bool XML4NLP::GetSentenceContent( string &content, int paragraphIdx, int sentenceIdx)
	{
		if (0 != CheckRange(paragraphIdx, sentenceIdx) ) return false;
		content = m_document_t.vecParagraph_t[paragraphIdx].vecSentence_t[sentenceIdx].sentencePtr->Attribute("cont");
		return true;
	}

	bool XML4NLP::GetSentenceContent( string &content, int globalSentIdx)
	{
		pair<int, int> paraIdx_sentIdx;
		if (0 != MapGlobalSentIdx2paraIdx_sentIdx(globalSentIdx, paraIdx_sentIdx)) return false;
		content = m_document_t.vecParagraph_t[paraIdx_sentIdx.first].vecSentence_t[paraIdx_sentIdx.second].sentencePtr->Attribute("cont");
		return true;
		
	}

	bool XML4NLP::AddSentence( const vector<Word> &wordList, int paragraphId )
	{
		if ( wordList.empty()){
			return false;
		}
		...
		if(sentenceNumber == -1 )
		{
			//throw domain_error("AddSentence error: Had been set over, can not be write again");
			throw "AddSentence error: Had been set over, can not be write again";
		}else if (sentenceNumber == 0){
			if (0 != BuildDOMFrame())
			{
				//throw domain_error("AddSentence error: DOM frame build error");
			...
			for(int i = 0; i< paragraphNumber; ++i){
				...
				m_document_t.vecParagraph_t.push_back(paraEle);
			}
		}else if (true == QueryNote(NOTE_SENT) && false == QueryNote(NOTE_WORD) && false == QueryNote(NOTE_POS) && false == QueryNote(NOTE_NE)
			&& false == QueryNote(NOTE_WSD) && false == QueryNote(NOTE_PARSER)){
				//throw invalid_argument("AddSentence error: Only sentence content can be set, there is no other attribute can be done");
		...
		{
			//throw out_of_range("AddSentence error: Paragraph id is out of the range!");
			throw "AddSentence error: Paragraph id is out of the range!";
			return false;
		}

		TiXmlElement* paraPtr = m_document_t.vecParagraph_t[paragraphId].paragraphPtr;
		vector<Sentence_t>& sentVec = m_document_t.vecParagraph_t[paragraphId].vecSentence_t;
	...
	//	printf("in\n");
		for (vector<Word>::const_iterator iter = wordList.begin(); iter != wordList.end(); ++iter)
		{
			...
			wordEle.wordPtr->SetAttribute(TAG_ID, sentEle.vecWord_t.size());
			if (QueryNote(NOTE_WORD))
			{
				wordEle.wordPtr->SetAttribute(TAG_CONT, iter->GetWS().c_str());
				contents += iter->GetWS();
			}
			if (QueryNote(NOTE_POS))
			{
				wordEle.wordPtr->SetAttribute(TAG_POS, iter->GetPOS().c_str());
			}
			if (QueryNote(NOTE_NE))
			{
				wordEle.wordPtr->SetAttribute(TAG_NE, iter->GetNE().c_str());
			}
			if(QueryNote(NOTE_WSD)){
				wordEle.wordPtr->SetAttribute(TAG_WSD, iter->GetWSD().c_str());
				wordEle.wordPtr->SetAttribute(TAG_WSD_EXP, iter->GetWSDExplanation().c_str());
			}
			if(QueryNote(NOTE_PARSER)){
				wordEle.wordPtr->SetAttribute(TAG_PSR_PARENT, iter->GetParserParent());
				wordEle.wordPtr->SetAttribute(TAG_PSR_RELATE, iter->GetParserRelation().c_str());
			}
	//		printf("insidee\n");

			sentEle.sentencePtr->LinkEndChild(wordEle.wordPtr);
	//		printf("insideb\n");
			sentEle.vecWord_t.push_back(wordEle);
		}

	//	printf("in\n");
		sentEle.sentencePtr->SetAttribute(TAG_ID, sentVec.size());
		sentEle.sentencePtr->SetAttribute(TAG_CONT, contents.c_str());
	...
	bool XML4NLP::AddSentence( const string& sentenceContent , int paragraphId)
	{
		...
		}else if(true == QueryNote(NOTE_SENT) && (true == QueryNote(NOTE_WORD) || true == QueryNote(NOTE_POS) || true == QueryNote(NOTE_NE)
			|| true == QueryNote(NOTE_WSD) || true == QueryNote(NOTE_PARSER)))	{
				//printf("in\n");
				//throw invalid_argument("AddSentence error: No word attribute can be used.");
				throw "AddSentence error: No word attribute can be used.";
				return false;
		}
		...
		sentEle.sentencePtr->SetAttribute(TAG_ID, sentVec.size());
		sentEle.sentencePtr->SetAttribute(TAG_CONT, sentenceContent.c_str());
	...
	bool XML4NLP::SetParagraphNumber( int paragraphNumber )
	{
		if (sentenceNumber != 0 || paragraphNumber < 1)
		{
			//throw invalid_argument("SetParagraphNumber error: Paragraph number can not be set again!");
			throw "SetParagraphNumber error: Paragraph number can not be set again!";
		}
		this->paragraphNumber = paragraphNumber;
		return true;
	}

	std::string XML4NLP::GetDOMstr()
	{
		string str;
		SaveDOM(str);
		return str;
	...
	bool XML4NLP::SetEncoding( const string& encodingType )
	{
		encoding = encodingType;
		return true;
	}

	bool XML4NLP::checkWordMatch( const Word & w )
	{
		if (QueryNote(NOTE_WORD) == true && w.HasWS() == false)
		{
			return false;
		}
		if (QueryNote(NOTE_POS) == true && w.HasPOS() == false)
		{
			return false;
		}
		if (QueryNote(NOTE_NE) == true && w.HasNE() == false)
		{
			return false;
		}
		if (QueryNote(NOTE_WSD) == true && w.HasWSD() == false)
		{
			return false;
		}
		if (QueryNote(NOTE_PARSER) == true && w.HasParser() == false)
		{
			return false;
		}
		return true;
	...
	void XML4NLP::SetOver()
	{
		sentenceNumber = -1;
	}

	void XML4NLP::checkEncoding()
	{
		/*
		TiXmlDeclaration* xmlDecl = m_tiXmlDoc.firstChild;
		if (xmlDecl == NULL)
		{
			cout<< "encoding null"<<endl;
		}
		else
		{
			cout<< "encoding null"<<xmlDecl<<endl;
		}
		//*/
	}
}
